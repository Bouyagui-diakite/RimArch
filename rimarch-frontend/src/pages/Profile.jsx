...
const roleColors = {
  ...
  archiviste: 'bg-blue-100 text-blue-700',
...
export default function Profile() {
  ...
  const Alert = ({ msg }) => {
    ...
    return (
      <div className={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm ${isSuccess ? 'bg-emerald-50 border border-emerald-200 text-emerald-700' : 'bg-red-50 border border-red-100 text-red-600'}`}>
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8">
      ...
      {/* Carte identité */}
      <div className="bg-white rounded-2xl border border-slate-200 p-7 flex items-center gap-6 shadow-sm">
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold shrink-0 shadow-md shadow-blue-200">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {user?.roles?.map((r) => (
              <span key={r.name} className={`text-xs font-medium px-2.5 py-1 rounded-full ${roleColors[r.name] || 'bg-slate-100 text-slate-600'}`}>
                {r.label}
              </span>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-7 py-5 border-b border-slate-100">
          <h2 className="text-base font-bold text-slate-800">Informations générales</h2>
        </div>
