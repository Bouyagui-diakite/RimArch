  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Journaux d'audit</h1>
        ...
        <div className="flex items-center gap-3">
          {hasFilters && (
            <button
              ...
              className="flex items-center gap-2 text-sm text-slate-500 hover:text-red-500 border border-slate-200 hover:border-red-200 px-4 py-2.5 rounded-xl transition-all"
            >
          ...
          <button
            ...
            disabled={exporting}
            className="flex items-center gap-2 bg-white hover:bg-slate-50 disabled:opacity-60 text-slate-700 text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors border border-slate-200 shadow-sm"
          >
            {exporting ? (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-600 font-semibold">Aucun log trouvé</p>
            <p className="text-slate-400 text-sm mt-1">Essayez d'autres filtres</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {logs.map((log) => {
              const action = ACTION_LABELS[log.action] || { label: log.action, color: 'bg-slate-100 text-slate-600 border-slate-200' }
              return (
                <div key={log.id} className="grid grid-cols-12 gap-4 px-8 py-4 items-center hover:bg-slate-50 transition-colors">
                  ...
                  {/* Description */}
                  <div className="col-span-4 text-sm text-slate-700 truncate">{log.description}</div>
                  ...
                  <div className="col-span-2 flex items-center gap-2 min-w-0">
                    {log.user ? (
                      <>
                        ...
                        <span className="text-sm text-slate-600 truncate">{log.user.name}</span>
                      </>
                    ) : (
                  ...
                  {/* Date */}
                  <div className="col-span-1 text-xs text-slate-400 text-right whitespace-nowrap">
      ...
      {meta.lastPage > 1 && (
        <div className="flex items-center justify-between px-1">
          <p className="text-sm text-slate-500">
            {meta.from}–{meta.to} sur {meta.total} événements
          </p>
